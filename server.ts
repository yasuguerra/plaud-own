...
export function sanitizeActionItems(items: any[]): any[] {
  if (!items || !Array.isArray(items)) return [];
  return items.map((item, index) => {
    const taskText = typeof item === "string" ? item : (item.task || "Nueva tarea");
    const importanceVal = item && item.importance ? item.importance : "medium";
    const idVal = item && item.id ? item.id : ("task_" + Math.random().toString(36).substring(3) + Date.now().toString(36) + "_" + index);
    const completedVal = item && typeof item.completed === "boolean" ? item.completed : false;
    const dueDateVal = item && item.dueDate ? item.dueDate : new Date().toLocaleDateString();
    
    return {
      id: idVal,
      task: taskText,
      importance: ["high", "medium", "low"].includes(importanceVal) ? (importanceVal as any) : "medium",
...
CRITICAL LANGUAGE REQUIREMENT: All output text (including title, summary, and action items) MUST be strictly in the detected language of the source transcript. If the transcript is in Spanish, the entire JSON output MUST be in Spanish. DO NOT respond in English if the source is in Spanish.`;
    ...
    const actionItems = sanitizeActionItems(stage2Result.actionItems || []);
...
app.post("/api/process", async (req, res) => {
  try {
    ...
    if (isSample) {
      ...
      if (parsedStudySession.actionItems) {
        parsedStudySession.actionItems = sanitizeActionItems(parsedStudySession.actionItems);
      }
...
app.get("/api/sessions/:id/media", async (req, res, next) => {
  try {
    ...
    const totalSize = metadata.size !== undefined ? parseInt(String(metadata.size), 10) : 0;
